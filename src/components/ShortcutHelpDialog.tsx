...
const TASK_KEYS: [string, string][] = [
  ["↑ / ↓", "行(タスク)を移動"],
  ["← / →", "列(項目)を移動"],
  ["h / j / k / l", "vim準拠でセル移動(h=左 / j=下 / k=上 / l=右)"],
  ...
  ["I", "中断(割り込み)"],
  ["W", "待ち ON/OFF"],
  ["C", "コピー(複製)。繰り返しは引き継がない"],
  ["Shift + C", "繰り返し設定ごとコピー(丸ごと複製)"],
  ["P", "次の日程へ延期(選択中は一括。繰り返しなしは翌営業日へ)"],
  ["Ctrl + H / L", "日付を前日 / 翌日へ(複数選択中は一括)"],
  ["Ctrl + K", "日付を今日へ(複数選択中は一括)"],
  ["Delete", "削除(複数選択中は一括)"],
];
