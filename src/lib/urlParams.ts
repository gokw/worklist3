...
//
//   **action は上の表示指定と併用できる**(#117)。読む経路が別々なので、
//   ?action=here&view=today なら「記録ダイアログを開いた上で、一覧は今日」になる。
//   manifest のショートカットもこの形にしてある。view は URL だけを拠り所にしていて
//   localStorage に残らないため、ふつうに開いたときの既定(今日以降)は変わらない。
//
//   ただし **mode だけは例外で、選んだ区分が localStorage に残る**(App.tsx)。
//   mode 付きのURLをブックマークすると、以後ふつうに開いたときもその区分のままに
//   なるので、ショートカットに mode を足すときはそれを承知で足すこと。
// ==============================================================
